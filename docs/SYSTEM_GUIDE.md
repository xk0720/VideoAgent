...
#### 5.1.6 LongVideoAgent (2025) — **同名工作，必须澄清关系**
- **链接**: https://arxiv.org/abs/2512.20618 (arXiv 2512.20618, late 2025)
- **解决什么**: 长视频 **VQA / reasoning**（不是剪辑）。master LLM 协调 grounding + vision agent，用 RL 训练 master 做"简洁正确高效"的协作。
- **跟我们的关系**: 名字几乎一样，但任务完全不同——他们 retrieve text 回答问题，我们 retrieve+generate shot 拼真 mp4。
- **我们采纳**: master-agent 用 step-limit 的设计 + RL training-of-master 思路（v0.4 roadmap）。
- **我们刻意不采**: 他们的 grounding agent 是 QA-tuned 而非剪辑-tuned；我们的 Director 不能直接套用。
- **行动项**: 文档里明确"LongVideoAgent" 名字撞车这一事实，避免后续读者混淆。

#### 5.1.7 GLANCE 2026 (formal publication)
- **链接**: https://arxiv.org/abs/2604.05076 (arXiv 2604.05076, 2026)
- **解决什么**: Music-Grounded Non-Linear Video Editing — 在长视频时间轴上生成开放式编辑（不是 QA 的 evidence retrieval）。
- **核心贡献**: bi-loop + 全局-局部协调 multi-agent；GPT-4o-mini backbone；比最强基线 +33.2% / +15.6%。
- **我们采纳**: 这是我们设计文档原本就提到的 GLANCE 的正式发表版；§5.1.4 的 bi-loop 描述对齐。
- **我们改造**: GLANCE 完全 LLM-driven，我们把外层 verify 一部分换成闭式 m1..m6 metric，降 cost。

#### 5.1.8 Sima 1.0 (2026)
- **链接**: arXiv 2604.07721
- **解决什么**: 多 agent **纪录片**自动化制作流水线，11 步分解，junior/senior agent 分级。
- **我们采纳**: stage 分解的"junior 做执行 senior 做审核"思路，对应我们 Editor (junior) vs Critic/Validator (senior) 的分工。
- **我们刻意不采**: 11 步过细——对剪辑场景 3 stage 已足够。
...
### 11.3 与 2025-2026 最新 self-loop 工作的对照（Round 11 增补）

我们的 5 机制 在 2026 年并不孤单——可以一一对应到 2025H2-2026H1 涌现出来的相关工作：

| 我们的机制 | 2025-2026 对应工作 | 关系 |
|---|---|---|
| LessonBook (跨 run 反思) | **Trajectory-Informed Memory Generation** (arXiv 2603.10600, 2026) | 同一思路；他们从 agent trajectory 生成结构化 memory entry，正是我们 CriticAgent→LessonBook flow |
| EnsembleRewardModel (K judge 投票) | **Multi-Agent Evolve (MAE)** Proposer/Solver/Judge (Oct 2025); **SELAUR** disagreement-as-reward (arXiv 2602.21158, 2026) | MAE 的 Judge 角色就是我们的 EnsembleRewardModel；SELAUR 用 disagreement 当 reward 信号，正好对接我们的 `EnsembleResult.disagreement` |
| PreferenceLogger (DPO 数据自动收) | **DGPO** (ICLR 2026, github.com/Luo-Yihong/DGPO); **GRPO ≡ DPO 证明** (arXiv 2510.00977) | DGPO 比 GRPO 更快，pairwise 数据格式一致；我们的 JSONL 同时能喂 DPO/IPO/KTO/SimPO/GRPO/DGPO 而不需要 schema 变更 |
| CriticAgent (post-hoc meta-critic) | **AgentPRM** (arXiv 2511.08325, late 2025) | AgentPRM 给每一步打 promise+progress 分；当我们 v0.3 训 RM 时，AgentPRM 是直接可借鉴的 recipe |
| Self-Consistency Screenwriter | **rStar-Math** MCTS over reasoning + verifier (Microsoft, Jan 2025); MAE Proposer (Oct 2025) | rStar-Math 是更高级的 sample-then-verify；我们 v0.3 可以把 majority-vote 升级为 verifier-based |

**关键观察**：领域内 2026 的明显趋势是"single-agent + RL → multi-agent + co-evolution"。我们 v0.1 起就是 multi-agent，Round 9 又补了 CriticAgent / Ensemble / PreferenceLogger，正好踏在这个趋势上。下一轮 v0.4 把 EditorAgent 真接 RL（用 RAGEN / ProRL Agent / Agent-R1 / MARTI 中任一框架），即可加入 MAE 那一支。

### 11.4 路线图 v0.3 / v0.4 / v0.5（每条都建立在这 5 个机制上）

> 详细的 RL 应用方案见独立文档 [`docs/AGENTIC_RL_PROPOSAL.md`](./AGENTIC_RL_PROPOSAL.md) — 含 2026 SOTA 调研 (40+ arXiv 引用)、三段式 (SFT→KTO→GRPO) recipe、风险与缓解、infra 选型、成功指标。本节只给出概要。
