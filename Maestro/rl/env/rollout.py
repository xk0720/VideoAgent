...
一次调用 = 一条轨迹:任务(剧本文件或 idea)→ 分镜 → 资产 → 逐镜
K 组采样 + skill 判官择主干 → outputs/movie_*/rl_steps.jsonl(reward
已内联,收集器只聚合)。
...
from env.clients import CallLog, KlingClient, TextLLM, WaveSpeedT2I  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--max-shots", type=int, default=16)
    ...
    if args.screenplay:
        ...
        if p.suffix == ".json":
            task_text = str(json.loads(p.read_text()).get("content",
                                                          "")).strip()
        else:
            task_text = p.read_text().strip()
    if not task_text:
    ...
    if not dashscope_key or not wavespeed_key:
        print("missing DASHSCOPE_API_KEY / WAVESPEED_API_KEY in .env",
              flush=True)
        return 2
    kling = KlingClient(dashscope_key,
    ...
    print(f"[env] run={run_dir.name} task={task_text[:40]!r} "
          f"policy={args.policy_model} "
          f"(v{os.environ.get('MAESTRO_POLICY_VERSION', '0')}) "
          f"K={args.group}", flush=True)
    res = run_episode(task_text=task_text, run_dir=run_dir,
                      frozen_llm=frozen_llm, policy=policy, kling=kling,
                      t2i=t2i, judges=judges, group=args.group,
