...
> **2026-06-11 更新：本文 §5-6 的 "sketch-as-oracle"（草图当验证预言机）定位已被
> v0.4 的参考自由像素验证取代**（见 `docs/research/survey_physics_2026_06.md` 与
> `docs/research/INNOVATION_PLAN_2026_06.md` §3.2）——"对比单条仿真轨迹"仍然预设了
> 单个 prompt 不可知的质量/摩擦/尺度参数（"你的仿真器才是错的"这一审稿攻击面没有
> 消除）；v0.4 把问题重写为参数自由的最佳运动定律拟合残差（"观测轨迹是否存在任何
> 物理一致的解释？"），从根上移除了这个攻击面。§1-4 的文献结论（控制线不可行、
> training-free 验证+搜索是正路）仍然成立，作为历史依据保留。
