...
def _fit_aspect(l, t, r, b, W, H, target_a):
    ...
    对齐目标 = 成片画布 9:16(用户裁决 2026-08-31)。依据: animate 输出画布跟随
    参考图的比例桶 —— 9:16 参考实测输出 704x1252(9:16, 比例正常, full_pro),
    非标准比例的参考会被挤进最近的桶, 人物随之变形(看片观感"被拉长");
    且 9:16 原生输出在拼装时零黑边满幅。
    为什么只动宽度: 景别由纵向范围定义(头到腰/膝/脚), 动高度等于换档 ——
